	...
	if (mysqli_num_rows($result) > 0)
	{
   	...
   	if(sizeof($arr) > 0)
   	{
		...
		echo 	"<div class=\"w3-center w3-card-12 w3-teal\">
				...
				<input class=\"w3-center w3-text-black w3-card-12\" type=\"text\" name = \"fullhisname\" style=\"width:600px;height:50px;\">	
				</div>
				<div class=\"w3-padding-bottom\">
		...
		foreach ($arr as $value)
   		{
			...
			$result = mysqli_query($conn, $sql);
   			while($row = mysqli_fetch_assoc($result))
			{
				if($row['remote_ID'] == $_SERVER['REMOTE_ADDR'] and $row['name'] == $value)
				{
					if($row['LB'] == 'L')
					{
